...
PUBLIC_CONF = ARGV[1] || "config/loon-rules-public.conf"
PRIVATE_CONF = ARGV[2] || "config/loon-private-nodes.conf"
RULE_DIR = ARGV[3] || "config/rules"
BASE_URL = ARGV[4] || "https://raw.githubusercontent.com/ifangng/loon-rules/main/config/rules"
...
def policy_to_file_stem(policy, index)
  POLICY_FILE_MAP[policy] || "policy-#{format('%02d', index + 1)}"
end
...
def split_rule(rule)
  ...
  policy_index = parts.length - 2 if parts.length >= 4 && TRAILING_FLAGS.include?(parts[-1].downcase)
...
def convert_proxy(proxy)
  ...
  when "trojan"
    ...
    raise "Unsupported proxy type: #{proxy['type']} (#{name})"
  end
end
...
def convert_group(group)
  ...
  case group["type"].to_s
  when "select"
    "#{name} = select, #{nodes.join(', ')}"
  when "url-test", "fallback"
    url = group["url"] || "http://www.gstatic.com/generate_204"
    interval = group["interval"] || 300
    "#{name} = #{group['type']}, #{nodes.join(', ')}, url=#{url}, interval=#{interval}"
  else
    raise "Unsupported proxy group type: #{group['type']} (#{name})"
  end
end
...
Dir.glob(File.join(RULE_DIR, "*.list")).each { |path| File.delete(path) }
...
policy_order.each_with_index do |policy, idx|
  ...
  if used_file_names[file_name]
    ...
    file_name = "#{stem}-#{used_file_names[file_name]}.list"
  else
    used_file_names[file_name] = 1
  end
  File.write(File.join(RULE_DIR, file_name), policy_to_rules[policy].join("\n") + "\n")
  manifest << [policy, file_name]
end

public_out = []
public_out << "[Rule]"
manifest.each { |policy, file_name| public_out << "RULE-SET,#{BASE_URL}/#{file_name},#{policy}" }
public_out << "FINAL,#{final_policy}"
File.write(PUBLIC_CONF, public_out.join("\n") + "\n")

private_out = []
private_out << "[Proxy]"
proxies.each { |p| private_out << convert_proxy(p) }
private_out << ""
private_out << "[Proxy Group]"
groups.each { |g| private_out << convert_group(g) }
File.write(PRIVATE_CONF, private_out.join("\n") + "\n")

puts "Done:"
puts "- public rules: #{PUBLIC_CONF}"
puts "- private nodes: #{PRIVATE_CONF}"
puts "- rule files: #{RULE_DIR} (#{manifest.size})"
